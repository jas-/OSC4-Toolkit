...
###############################################
# Acquire a list of priority systems that indicate both nodes as OFFLINE
###############################################
...
critical_report=( $(echo "${report[@]}" | tr ' ' '\n' | egrep ${monitor_filter} | 
  awk -F, '$4 ~ /.*:.*:Offline:.*:.*:Offline/{printf("CRITICAL,Application,%s,(%s),%s,(%s),%s,(%s)\n", $3, $4, $6, $7, $9, $10)}' |
  sed 's/:,/,/g' | sed 's/\^/ /g') )


###############################################
# Acquire a list of non-priority systems that indicate both nodes as OFFLINE
###############################################
...
general_report=( $(echo "${report[@]}" | tr ' ' '\n' | egrep -v ${monitor_filter} | 
  awk -F, '$4 ~ /.*:.*:Offline:.*:.*:Offline/{printf("LOW,Application,%s,(%s),%s,(%s),%s,(%s)\n", $3, $4, $6, $7, $9, $10)}' |
  sed 's/:,/,/g' | sed 's/\^/ /g') )
